...
@section('content')
    ...
    <div class="content-header">
        <h4 class="title is-4">Tambah Materi Baru</h4>
        <span class="separator"></span>
        <nav class="breadcrumb has-bullet-separator" aria-label="breadcrumbs">
            <ul>
                <li><a href="{{ route('materis.index') }}">Materi</a></li>
                <li class="is-active"><a href="#" aria-current="page">Tambah Data</a></li>
            </ul>
        </nav>
    </div>
            ...
            <div class="card-content">
                ...
                <form action="{{ route('materis.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="select is-fullwidth @error('id_kurikulum') is-danger @enderror">
                                <select name="id_kurikulum" required>
                                    ...
                                    @foreach ($kurikulums as $kurikulum)
                                        <option value="{{ $kurikulum->id_kurikulum }}" {{ old('id_kurikulum') == $kurikulum->id_kurikulum ? 'selected' : '' }}>
                                            {{ $kurikulum->nama }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="field">
                        ...
                        <div class="control">
                            <input type="text" name="judul" class="input @error('judul') is-danger @enderror" placeholder="Contoh: Pengenalan Aljabar" value="{{ old('judul') }}" required>
                        </div>
                        @error('judul')
                            <p class="help is-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="field">
                        ...
                        <div class="control">
                            <input type="number" name="urutan" class="input @error('urutan') is-danger @enderror" placeholder="Contoh: 1" value="{{ old('urutan', 0) }}" required>
                        </div>
                    ...
                    <div class="field">
                        ...
                        <div class="control">
                            <textarea class="textarea @error('keterangan') is-danger @enderror" name="keterangan" placeholder="Deskripsi singkat materi">{{ old('keterangan') }}</textarea>
                        </div>
                        @error('keterangan')
                            <p class="help is-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="field">
                        ...
                        <div class="control">
                            <textarea class="textarea @error('rangkuman') is-danger @enderror" style="height:150px" name="rangkuman" placeholder="Rangkuman detail materi">{{ old('rangkuman') }}</textarea>
                        </div>
                        @error('rangkuman')
                            <p class="help is-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="field">
                        ...
                        <div class="control has-icons-left">
                            <input type="url" name="youtube" class="input @error('youtube') is-danger @enderror" placeholder="https://www.youtube.com/watch?v=xxxxxxxxxxx" value="{{ old('youtube') }}">
    ...
    document.querySelectorAll('.file-input').forEach(input => {
        input.onchange = () => {
            if (input.files.length > 0) {
                ...
                // Reset nama file jika tidak ada file yang dipilih (misalnya setelah menghapus pilihan)
                const fileName = input.closest('.file').querySelector('.file-name');
                fileName.textContent = 'Tidak ada file terpilih';
            }
        }
    });
